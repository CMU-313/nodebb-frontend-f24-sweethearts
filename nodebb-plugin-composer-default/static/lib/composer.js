...
define('composer', [
	...
	function handleAnonymous(postContainer) { 
		const anonymousToggle = postContainer.find('#anonymous-toggle');
		
		anonymousToggle.on('click', function () {
		...
		api[method](route, composerData)
			.then((data) => {
				...
				if (data.queued) {
					...
					//const pid = data.mainPid;
					// const authToken = localStorage.getItem('token');
					// fetch('/api/v3/posts/' + pid, {
					// 	method: 'PUT',
					// 	headers: {
